...
# در صورت تمایل توکن ربات را تغییر دهید
...
def load_db():
    default_db = {
        ...
        "settings": {
            ...
            # تنظیمات جدید اضافه‌شده (بدون آسیب به ساختار قبلی)
            "v2ray_rename": {
                "enabled": False,
                "name": "ProxyArc",
...
def update_queue(current_list, new_items, max_limit, delete_batch):
    ...
    gc.collect() # جلوگیری از پر شدن RAM
...
def get_base_url():
    # بررسی هوشمند متغیرهای محیطی سرویس‌های مختلف
    envs = [
    ...
    # تلاش برای استخراج از Context ریکوئست Flask
    try:
        if request:
        ...
        pass # خارج از Context درخواست است
...
@app.route('/health')
def health_check():
    # رفع خطای Apply.Build و 502 Bad Gateway
...
def get_main_keyboard():
    ...
    # دکمه‌های جدید بدون حذف قبلی‌ها
    markup.add(
...
def send_files_to_chat(chat_id, file_type="both"):
    ...
    if file_type in ["v2ray", "both"] and db["v2ray"]:
        clean = deduplicate_list(db["v2ray"])
        processed = process_v2ray_links(clean)
        f_v2ray = BytesIO("\n".join(processed).encode()) # خروجی خام و غیر Base64
...
def callback_inline(call):
    ...
    try:
        # ---- اضافه شده برای فایل و شخصی سازی ----
...
def run_telegram_bot():
    ...
    # Polling با تنظیمات مقاوم در برابر قطعی اینترنت
    bot.infinity_polling(timeout=10, long_polling_timeout=5, logger_level=logging.ERROR)

if __name__ == "__main__":
    # استفاده از Wrapper های فناناپذیر بجای Thread ساده
    ...
    # استفاده از threaded=True برای جلوگیری از 502 Bad Gateway
